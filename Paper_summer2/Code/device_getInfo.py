...
if __name__ == '__main__':
    ...
    outputInfo = ''
    extraInfo = ''
    # recFile = open("device_communication_attack:10Th_.txt", "a")
    ...
    try:
        ...
        for item in stateArr:
            if item == 1: 
                numLoss += 1
        finalInfo = 'Total-Times:' + str(lossCounter+counterd-1) + '\t' + 'Loss-Times:' + str(lossCounter) + '\t' + 'Recent 10 times loss possiblity: 0.' + str(numLoss) + '\t' + 'Total Hex File Loss number: ' + str(numFileLoss)
        print(finalInfo)
        # recFile.write('\n'+finalInfo)
        # recFile.close()
        print '>>>quit'
